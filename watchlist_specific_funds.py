...
def watchlist_specific_funds():
    if "username" in session:
        ...
        sql_non_listed = text("""SELECT fund_name 
                                FROM fund 
                                WHERE id NOT IN (SELECT fund_id FROM watchlist WHERE user_id = :user_id)""")
...
def chosen_watchlist_funds():
    if "username" in session:
        ...
        result = db.session.execute(sql_id, {"username": username})
        user_id = result.fetchone()[0]
        sql_listed = text("""SELECT fund_name 
                          FROM fund 
                          WHERE id IN (SELECT fund_id FROM watchlist WHERE user_id = :user_id)""")
...
def watchlist_sums():
    if "username" in session:
        ...
        result = db.session.execute(sql_id, {"username": username})
        user_id = result.fetchone()[0]
        sql_watchlist = text("""SELECT f.fund_name, COALESCE(SUM(t.amount), 0) as total_amount 
                                FROM fund f 
